...
FSWriteSendData:
		ldx 	#1 							; current sub-page to write to.
_FSWSDLoop:
		lda 	shDataSize 					; complete
		ora 	shDataSize+1
		beq 	_FSWSDExit

		jsr 	FSIncrementSetLoad 			; bump address, copy original to iTemp0
		lda 	currentSector 				; write to current sector and page
		phx
		jsr 	FSHWrite
		plx
		inx 								; next sub page
		dec 	shDataSize+1 				; gone to $FF ($00 will check at top.)
		lda  	shDataSize+1
		cmp 	#$FF
		bne 	_FSWSDLoop
_FSWSDExit:		
